...
import { useSuperAdminLogin } from "../hooks/useAdminAuth";

export function SuperAdminLoginPage() {
  const [showPassword, setShowPassword] = useState(false);
  const { form, onSubmit, isSubmitting, formError } = useSuperAdminLogin();
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-white font-sans antialiased text-[#111827]">
      ...
      <section className="w-full md:w-[40%] bg-gradient-to-br from-[#0f172a] to-[#4f46e5] p-8 md:p-12 flex flex-col justify-between items-start text-white relative overflow-hidden">
        ...
        {/* Glass Card */}
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-6 md:p-8 max-w-md w-full relative z-10 shadow-xl">
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-3">
            Manage the FinScrybe Platform
          </h1>
          <p className="text-white/80 text-sm leading-relaxed">
            Monitor companies, subscriptions, platform performance, claims, and business growth. Access full system logs and enterprise-grade controls.
          </p>
        </div>

        {/* 3D Illustration Graphic */}
        <div className="w-full max-w-md mx-auto my-10 relative z-10 flex items-center justify-center">
          <img 
            src={loginPanelBg} 
            alt="FinScrybe Platforms Illustration" 
            className="w-full h-auto object-contain opacity-95 transition-opacity scale-110" 
          />
        </div>
      ...
      <main className="w-full md:w-[60%] flex flex-col justify-between items-center px-6 py-12 md:p-12 overflow-y-auto bg-[#f8fafc]">
        <div className="max-w-md w-full my-auto">
          ...
          <div className="flex flex-col items-center text-center mb-8">
            ...
            <div className="inline-flex items-center gap-1.5 bg-indigo-50 border border-indigo-100 text-[#635BFF] text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full">
              <ShieldCheck size={12} />
              SUPER ADMIN PORTAL
            </div>
          </div>
          ...
          <div className="text-center mb-8">
            <h2 className="text-2xl md:text-3xl font-extrabold text-slate-950 tracking-tight">Super Admin Login</h2>
            <p className="text-slate-500 mt-2 text-sm font-medium">Login to manage FinScrybe companies.</p>
          </div>
            ...
            <form onSubmit={onSubmit} className="space-y-5" noValidate>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    id="email"
                    type="email"
                    placeholder="admin@finscrybe.io"
              ...
              <div className="flex items-center justify-between text-xs py-1">
                ...
                <a 
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    alert("Forgot password function is locked. Contact server administrators.");
                  }} 
          ...
          <div className="text-center mt-6">
            <Link to="/" className="text-sm font-bold text-slate-500 hover:text-slate-700 transition-colors inline-flex items-center gap-1.5">
        ...
        <footer className="w-full flex justify-between text-[11px] font-semibold text-slate-400 pt-8 border-t border-slate-200 max-w-lg">
          <span>© 2026 FinScrybe Financial Intelligence.</span>
          <div className="flex gap-4">
            <a href="#sec" onClick={(e) => e.preventDefault()} className="hover:underline">Security Architecture</a>
            <a href="#sys" onClick={(e) => e.preventDefault()} className="hover:underline">Contact Systems</a>
          </div>
        </footer>
      </main>
    </div>
