...
export function SignupPage() {
  ...
  const handleSignup = (e: React.FormEvent) => {
    ...
    setError(null);
    setClientUser({ email, fullName });
    // Redirect to client dashboard on successful signup
    navigate("/dashboard");
  };
      ...
      <main className="w-full md:w-[55%] flex flex-col justify-between items-center px-6 py-12 md:p-12 overflow-y-auto">
        <div className="max-w-md w-full my-auto">
          <header className="mb-8">
            <h2 className="text-3xl font-extrabold text-slate-950 tracking-tight">Create Your Account</h2>
            <p className="text-slate-500 mt-2 font-medium">Start your journey with FinScrybe.</p>
          </header>
          ...
          <form className="space-y-5" onSubmit={handleSignup}>
            {/* Full Name */}
            <div>
              <label htmlFor="fullName" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                Full Name
              </label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  type="password"
                  placeholder="••••••••"
                  className="w-full px-4 py-3 bg-[#f8fafc] border border-slate-200 rounded-xl font-medium placeholder-slate-400 text-slate-900 focus:outline-none focus:border-[#635BFF] focus:bg-white transition-all text-sm"
                  value={password}
              ...
              <div>
                <label htmlFor="confirmPass" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                  Confirm Password
                </label>
                <input
                  id="confirmPass"
            ...
            <div className="flex items-start gap-3 py-2">
              <input
                id="agree"
                type="checkbox"
                className="mt-1 w-4 h-4 rounded text-[#635BFF] border-slate-300 focus:ring-[#635BFF]"
                checked={agree}
                onChange={(e) => setAgree(e.target.checked)}
              />
              <label htmlFor="agree" className="text-xs font-semibold text-slate-500 leading-normal">
                I agree to the <a href="#terms" className="text-[#635BFF] hover:underline">Terms of Service</a> and <a href="#privacy" className="text-[#635BFF] hover:underline">Privacy Policy</a>.
              </label>
            </div>
          ...
          <p className="text-sm font-medium text-slate-500 text-center">
            ...
            <Link to="/login" className="text-[#635BFF] hover:underline font-bold">
              Sign In
            </Link>
          </p>
        </div>
